...
func main() {
	...
	//Reader configurato per ricevere messaggi di conferma dal leader
	readerFromLeader := kafka.NewReader(configRead)
	...
	//Creazione canale di comunicazione "privato" del nodo in cui potrà ricevere l'autorizzazione o messaggi di heart beat
	...
	for i := 0; i < len(ALL_ARTICLE.Articles); {
		...
		handlerLeaderComunication := H.LeaderComunicationHandler{Url: URLKAFKA, ID_LEADER: leaderID, ID_NODE: ID_NODE, ReaderLeader: readerFromLeader}
